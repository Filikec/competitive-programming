...
class Treap{
    public:
    typedef struct node{
        long long count, nodes, key, priority; //count = how many key there are, nodes = how many children + 1 (node intself)
    ...
    void insert(long long key, long long priority=-1, int count=1){ // inserts key into tree, if key already exists, increases count
...
int main(){
    int n;
    before = new Treap, after= new Treap;
    ...
    for (int i=0;i<n;i++){
        long long more, less=0;

        before->insert(Nums[i],before->Root->priority+1);
        after->push_to_top(after->find(Nums[i]));

        more = before->Root->right->nodes;
        less = after->Root->left->nodes;

        after->remove(Nums[i]);

        combination += more*less;
    }
