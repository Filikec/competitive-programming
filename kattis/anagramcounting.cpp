...
int main(){
    ...
    while (getline(cin,line)){
        ...
        for (auto c : line){
            ...
            combination = factorial[line.size()];
            BigNum div = 1;
            for (auto c : chars){
                div = div.multiply(factorial[c.second]);
            }
            combination = combination.multiply(div.inverse());
        }
        cout << combination.numerator << '\n';
    }
