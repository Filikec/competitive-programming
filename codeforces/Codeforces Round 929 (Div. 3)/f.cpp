...
bool isRock(int time, int i, int j, int n){
    return G[(i+time)%n][j];
}
...
void work(){
    ...
    while (q.size()){
        ...
        if (j == m-1){
            best = min(best,min((n-1)-i,i+1)+curT);
        }

        int newI = (i+1)%n;
        pii newPos = {newI,j};
        if (!isRock(curT+1,newI,j,n) && !isRock(curT,newI,j,n) && !u.count(newPos)) q.push_back(newPos), u[newPos]=curT+1;

        if (j+1 < m){
            newPos = {i,j+1};
            if (!isRock(curT+1,i,j+1,n) && !u.count(newPos)) q.push_back(newPos),u[newPos]=curT+1;
        }
    }
    if (best == 1e10) cout << "-1\n";
    else cout << best << '\n';
