...
void work(){
    ...
    for (int i=0;i<q;i++){
        ...
        while (hi-lo > 3){
            ...
            ll items_left = s[left+1]-s[l-1];
            ll items_right = s[right+1]-s[l-1];


            if (getPerformance(items_left,u) < getPerformance(items_right,u)) lo = left;
            else hi = right;
        }
        ...
        ll best = lo;
        for (ll i=lo;i<=hi;i++){
            ll items_left = s[best+1]-s[l-1];
            ll items_right = s[i+1]-s[l-1];
            if (getPerformance(items_left,u) < getPerformance(items_right,u)) best = i;
        }
        cout << best+1 << ' ';
    }
    cout << '\n';
